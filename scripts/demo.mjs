...
try {
  await visit('/', 'Enxergue o mundo');
  await page.screenshot({ path: path.join(output, 'home-desktop.png'), fullPage: true });
  await page.mouse.wheel(0, 670);
  await page.waitForTimeout(750);
  await visit('/catalogo', 'Uma coleção para');
  ...
  await visit('/sobre', 'Uma história feita');
  await visit('/contato', 'Estamos aqui para');
  assert.equal(await page.locator('iframe[title="Mapa da Sul Ótica em Varginha"]').count(), 1);
  ...
  await mobilePage.getByRole('navigation', { name: 'Navegação móvel' }).getByRole('link', { name: 'Catálogo' }).click();
  await mobilePage.getByRole('heading', { name: 'Uma coleção para', exact: false }).waitFor();
  ...
  await mobilePage.getByRole('button', { name: 'Testar sem usar cota' }).click();
  ...
  console.log('Routes, filters, local face analysis, WhatsApp links, map and mobile menu: OK');
} finally {
