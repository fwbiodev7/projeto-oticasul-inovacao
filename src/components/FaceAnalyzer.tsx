...
export function FaceAnalyzer() {
  ...
  return <div id="experimente" className="scroll-mt-28">
    ...
    {stage === 'result' && result ? <FaceResult result={result} onRestart={restart} /> : <div className="mx-auto max-w-3xl rounded-[1.75rem] border border-primary/10 bg-white p-5 shadow-soft sm:p-9">
      ...
      {stage === 'preview' && image && <><div className="mb-6 flex items-center justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-accent">Sua foto</p><h2 className="mt-2 text-2xl font-semibold text-primary">Pronto para descobrir?</h2></div><button type="button" onClick={restart} className="flex items-center gap-1 text-xs font-semibold text-primary/60 hover:text-accent"><RotateCcw size={15} />Trocar foto</button></div><div className="relative mx-auto aspect-[4/3] max-w-sm overflow-hidden rounded-2xl bg-light"><Image src={image} alt="Prévia da foto enviada" fill unoptimized sizes="384px" className="object-contain" /></div><div className="mt-6 flex flex-wrap justify-center gap-3"><button type="button" onClick={() => analyze('gemini')} className="btn-primary"><Sparkles size={18} />Analisar com Gemini<ArrowRight size={17} /></button><button type="button" onClick={() => analyze('local')} className="btn-outline"><ScanFace size={18} />Testar sem usar cota</button></div><p className="mt-5 text-center text-xs leading-6 text-ink/55">Com Gemini, a foto é enviada ao Google. No teste local, os pontos do rosto são analisados no seu aparelho. Se a API estiver indisponível, usamos automaticamente a análise local. Nenhuma foto é salva pelo protótipo.</p></>}
