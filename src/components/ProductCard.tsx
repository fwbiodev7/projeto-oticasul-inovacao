...
export function ProductCard({ product }: { product: Product }) {
  return <article className="group overflow-hidden rounded-[1.35rem] border border-primary/5 bg-white shadow-[0_6px_25px_rgba(10,55,102,.035)] transition-all duration-300 hover:-translate-y-1.5 hover:shadow-[0_20px_40px_rgba(6,51,92,.12)] hover:border-accent/30">
    <div className="relative aspect-[1.08] overflow-hidden bg-[#f3f8fa]">
      <Image src={product.image} alt={`Armação ${product.name}`} fill sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 25vw" className="object-cover transition-transform duration-700 ease-out group-hover:scale-110" />
